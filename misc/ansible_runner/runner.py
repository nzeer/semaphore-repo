...
from .exceptions import AnsibleRunnerException, CallbackError
from .utils import (
    OutputEventFilter,
...
class Runner:
    def __init__(
        ...
        self.runner_mode = (
            self.config.runner_mode
            if hasattr(self.config, "runner_mode")
            else "pexpect"
        )

        self.directory_isolation_path = (
            self.config.directory_isolation_path
            if hasattr(self.config, "directory_isolation_path")
            else None
        )
        self.directory_isolation_cleanup = (
            self.config.directory_isolation_cleanup
            if hasattr(self.config, "directory_isolation_cleanup")
            else None
        )
        self.process_isolation = (
            self.config.process_isolation
            if hasattr(self.config, "process_isolation")
            else None
        )
        self.process_isolation_path_actual = (
            self.config.process_isolation_path_actual
            if hasattr(self.config, "process_isolation_path_actual")
            else None
        )
    ...
    def event_callback(self, event_data):
        ...
        if "uuid" in event_data:
            ...
            partial_filename = os.path.join(
                self.config.artifact_dir, "job_events", filename
            )
            ...
            try:
                ...
                try:
                    with codecs.open(
                        partial_filename, "r", encoding="utf-8"
                    ) as read_file:
                ...
                if should_write:
                    ...
                    with codecs.open(
                        temporary_filename, "w", encoding="utf-8"
                    ) as write_file:
    ...
    def status_callback(self, status):
        ...
        if status == "starting":
            ...
            status_data.update({"command": self.config.command, "cwd": self.config.cwd})
        for plugin in ansible_runner.plugins:
    ...
    def run(self):
        ...
        with codecs.open(command_filename, "w", encoding="utf-8") as f:
            ...
            json.dump(
                {"command": command, "cwd": self.config.cwd},
                f,
                ensure_ascii=False,
            )
        ...
        if self.runner_mode == "subprocess":
            ...
            subprocess_timeout = (
                ...
                if hasattr(self.config, "subprocess_timeout")
                else None
            )
            try:
                stdout_response = ""
            ...
            while child.isalive():
                ...
                if (
                    ...
                    and (time.time() - self.last_stdout_update)
                    > self.config.idle_timeout
                ):
                    self.kill_container()
                    Runner.handle_termination(child.pid)
        ...
        for filename, data in [
            ...
            if not os.path.exists(artifact_path):
                os.close(
                    os.open(artifact_path, os.O_CREAT, stat.S_IRUSR | stat.S_IWUSR)
                )
        ...
        while not os.path.exists(event_path):
            ...
            if wait_time.total_seconds() > 60:
                raise AnsibleRunnerException(
                    f"events directory is missing: {event_path}"
                )
